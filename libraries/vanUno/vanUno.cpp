...
void Elog(uint8_t file, uint8_t errorcode) {
	...
	Message buff;
	buff.set(STD, THISNODE, ELOGDEST, MSGERROR, file, errorcode, true);
...
void vanWrite(uint8_t outbyte) {
	...
	//uno/nano has only one serial port: 0. Any other is erroneous 
...
void handleMessage(Message inData) {
	...
	Elog(FNUM,4);//ERROR- Message has destination unknown to this node
}
...
Message getMessage(int selected) {
	...
	buff.start = start;	  //start byte must be kept retain ACK type of message
